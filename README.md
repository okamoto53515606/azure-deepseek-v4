...
| 運用負荷 | 高い（ハードウェア、ドライバ、GPU メモリ、電源、冷却、障害対応） | 低い | 低い（クラウド事業者が運用） |
...
4. 社内ですでに East US の ACA PoC で **同じモデル・同じ `/openai/v1/`** を使っている。手順の再現性が高い。
...
- 対象: Azure や Python の操作に慣れていない情報システム部門の担当者
- 用途: 社内 PoC（動作検証）。本番導入手順ではない
- 作成日: 2026-09-11（第2部として 2026-09-12 更新）
...
**動作確認（2026-09-11）:** サービスアカウント認証による `SELECT 1` が成功済み。資料には実在するプロジェクト ID、アカウント名、リソース名、鍵 ID を記録しない。
